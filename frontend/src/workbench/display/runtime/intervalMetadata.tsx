...
export function buildBoreholeMetadata(data: BoreholeWorkbench) {
  ...
  return [
    ...
    { label: "Reduced level", value: String(collar.reduced_level ?? metadata.reduced_level ?? metadata.rl ?? "-") },
    { label: "Water level", value: String(collar.water_level ?? metadata.water_level ?? boreholeAttributes.water_level ?? "-") },
    { label: "Total depth", value: `${data.total_depth} m` },
    { label: "Curves", value: String(data.curves.length) },
    { label: "Core images", value: data.core_images.length ? String(data.core_images.length) : "Not supplied" },
    { label: "Source files", value: String(data.source_files.length) },
    { label: "Import batches", value: String(data.source_imports.length) },
